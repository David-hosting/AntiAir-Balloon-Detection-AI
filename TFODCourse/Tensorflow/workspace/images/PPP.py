# import os

# folder_path = 'C:\\Users\\User\\Desktop\\local\\TFODCourse\\Tensorflow\\workspace\\images\\train'

# for filename in os.listdir(folder_path):
#     if not (filename.endswith('.jpeg') or filename.endswith('.xml')):
#         print(f'{filename} is not a PNG or JPEG file')

from pathlib import Path
import imghdr
import tensorflow as tf
import os

img_link=list(Path("C:\\Users\\User\\Desktop\\local\\TFODCourse\\Tensorflow\\workspace\\images\\test").glob(r'**/*.jpg'))

count_num=0
for lnk in img_link:
    with open(lnk,'rb') as binary_img:
        find_img=tf.compat.as_bytes('JFIF') in binary_img.peek(10)#The JFIF is a JPEG File Interchange Format (JFIF). It is a standard which we gauge if an image is corrupt or substandard
    if not find_img:
        count_num+=1
        os.remove(str(lnk))
print('Total %d pcs image delete from Dataset' % count_num)
#this should help you delete the bad encoded